...
#   Version 50 - Full Port of Bash Version
# ============================================
...
clear
printf "\n%sChromebook WP Diagnostic v50 (VT3) by CriticalHD/neonnebula%s\n" "$CYAN" "$NC"
printf "%s=========================================%s\n" "$YELLOW" "$NC"
...
print_diagnostics() {
    ...
    [ "$dev_usb" -eq 1 ] && dev_usb_status="${GREEN}ENABLED${NC}" || dev_usb_status="${RED}DISABLED${NC}"
    ...
    if command -v flashrom >/dev/null 2>&1 && command -v futility >/dev/null 2>&1; then
        ...
        [ "$gbb_flags" != "0x0" ] && gbb_dev_usb=$(printf "%d" "$gbb_flags")
    fi
    printf "%sGBB Flags = %s\n" "$YELLOW" "$gbb_flags"
    ...
    while kill -0 $FR_PID 2>/dev/null; do
        i=$(( (i+1) % 4 ))
        printf "\r\033[1;33mReading SPI flash... %s\033[0m" "$(printf "%c" "${spinner:$i:1}")"
        sleep 0.2
    done
    wait $FR_PID
    FR_STATUS=$?
