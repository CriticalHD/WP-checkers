...
#   Version 49 - Crossystem Numeric + Enabled/Disabled
# ============================================
...
# ---------- HEADER ----------
clear
printf "\n%sChromebook WP Diagnostic v49 (VT3) by CriticalHD/neonnebula%s\n" "$CYAN" "$NC"
printf "%s=========================================%s\n" "$YELLOW" "$NC"
...
# Numeric + status
wpsw_status=$([ "$wpsw" = "1" ] && echo "${RED}WP ENABLED${NC}" || echo "${GREEN}WP DISABLED${NC}")
devsw_status=$([ "$devsw" = "1" ] && echo "${GREEN}ENABLED${NC}" || echo "${RED}DISABLED${NC}")
dev_usb_status=$([ "$dev_usb" = "1" ] && echo "${GREEN}ENABLED${NC}" || echo "${RED}DISABLED${NC}")
dev_signed_status=$([ "$dev_signed" = "1" ] && echo "${RED}SIGNED ONLY ENFORCED${NC}" || echo "${GREEN}UNSIGNED ALLOWED${NC}")
...
if command -v flashrom >/dev/null 2>&1 && command -v futility >/dev/null 2>&1; then
    ...
    printf "%sGBB Flags = %s → %s\n" "$YELLOW" "$gbb_flags" "$CYAN"

    if [ $((gbb_flags_dec & 16)) -ne 0 ]; then
        printf "%sGBB Dev USB = FORCED ENABLED%s\n" "$GREEN" "$NC"
    else
        printf "%sGBB Dev USB = Not Forced / Disabled%s\n" "$RED" "$NC"
    fi
else
...
if command -v flashrom >/dev/null 2>&1; then
    ...
    case "$mode" in
    disabled)
        printf "%sRO = WRITABLE%s\n" "$GREEN" "$NC"
        printf "%sRW = WRITABLE%s\n" "$GREEN" "$NC"
        ;;
    hardware)
        printf "%sRO = PROTECTED (hardware)%s\n" "$RED" "$NC"
        printf "%sRW = LIKELY WRITABLE%s\n" "$YELLOW" "$NC"
        ;;
    software)
        printf "%sRO = PROTECTED (firmware)%s\n" "$YELLOW" "$NC"
        printf "%sRW = POSSIBLY WRITABLE%s\n" "$GREEN" "$NC"
        ;;
    *)
        printf "%sRO/RW unknown%s\n" "$MAGENTA" "$NC"
        ;;
    esac
else
